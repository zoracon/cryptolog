...
fn main() -> Result<()> {
    ...
    let hashed_size = 6;
    // let hashed = hash(log_entry, 6, None);
    ...
    let data: Vec<String> = buffered
            ...
            .map(|line| set.replace_all(&line, &hash(line.as_bytes(), 6, None)).to_string())  // encrypt entry and rebuild...
            .collect();
    ...
    // .for_each(|x| println!("{}", x)); for debugging
